...
export default function HomePage(props: any) {
  ...
  useEffect(() => {
    ...
    updateParserState();
    http_common
      .get<IPlatform[]>("Platform/getAllPlatforms")
      .then((data) => {
        //console.log(data);
        //data.data =
        data.data.shift();
  ...
  const updatePlatforms = (el: IPlatform) => {
    ...
    const elementIndex: number | undefined = newPlatforms.findIndex(
      (el2) => el2.id === el.id
    );
    if (elementIndex !== undefined) {
      ...
      newPlatforms[elementIndex].isSelected =
        !newPlatforms[elementIndex].isSelected;
  ...
  return (
    <div className="row p-5">
      {loading ? (
        <Spinner animation="border" className="p-5 text-center"></Spinner>
      ) : (
        <div className="offset-md-3 col-md-6">
          <button
            className="btn btn-primary mb-2"
            onClick={() => updateParserState()}
          >
            {" "}
            Refresh <FontAwesomeIcon icon={faArrowRotateForward} />{" "}
          </button>
